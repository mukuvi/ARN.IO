...
router.get("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    const chapters = (
      await pool.query(
        "SELECT chapter_number,title FROM chapters WHERE book_id=$1 ORDER BY chapter_number",
        [book.id],
      )
    ).rows;
...
router.get("/:id/full-text", authenticateToken, async (req, res) => {
  try {
    ...
    if (!book.full_text)
      return res
        .status(404)
        .json({ error: "Full text not available for this book" });
...
router.get("/search-online/:query", authenticateToken, async (req, res) => {
  try {
    ...
    async function searchGoogleBooks() {
      ...
      const url = `https://www.googleapis.com/books/v1/volumes?q=${query}&maxResults=12&printType=books${keyPart}`;
      ...
      if (!response.ok || data?.error) {
        ...
        const message =
          data?.error?.message || response.statusText || "Google Books error";
      ...
      const books = (data.items || []).map((item) => {
        ...
        return {
          ...
          cover_url:
            ...
            `https://api.dicebear.com/9.x/shapes/svg?seed=${encodeURIComponent(title)}&backgroundColor=f97316,ef4444,8b5cf6,3b82f6,10b981,f59e0b&size=200`,
    ...
    async function searchOpenLibrary() {
      const url = `https://openlibrary.org/search.json?q=${query}&limit=12&fields=key,title,author_name,first_publish_year,cover_i,subject`;
      const response = await fetch(url, { signal: AbortSignal.timeout(6000) });
      ...
      const books = (data?.docs || []).map((doc) => {
        ...
        const infoLink = workKey ? `https://openlibrary.org${workKey}` : null;
        return {
          id: `openlibrary_${workKey || encodeURIComponent(title)}`,
          title,
          ...
          description: "",
          pages: 0,
          published_year: doc.first_publish_year || 0,
          rating: 0,
    ...
    let books = [];
    try {
      books = await searchGoogleBooks();
    } catch (err) {
      // Google Books unauthenticated requests can be rate-limited (429). Fall back to Open Library.
      console.warn(
        `Google Books search failed (${err.status || "?"}): ${err.message}`,
      );
          ...
          messages: [
            {
              role: "system",
              content:
                "You detect book authors. Respond with ONLY the author's name — nothing else. If you cannot determine the author, respond with exactly: Unknown",
            },
            {
              role: "user",
              content: `Who is the author of this book?\n\nTitle: "${title}"\n\n--- BOOK TEXT (first pages) ---\n${excerpt}`,
            },
          ],
          temperature: 0.1,
          max_tokens: 100,
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a precise book analysis assistant. Always respond with valid JSON arrays only. No markdown, no backticks, no extra text.",
            },
...
function basicChapterGrouping(text) {
  ...
  return chapters.length > 0
    ? chapters
    : [{ title: "Full Text", content: text.trim() }];
}
  ...
  async (req, res) => {
    try {
      ...
      if (
        !author ||
        author.trim() === "" ||
        author.trim().toLowerCase() === "unknown"
      ) {
      ...
      const result = await pool.query(
        ...
        [
          ...
          genre || "Personal",
          coverUrl,
          `~${estimatedPages} pages, ~${wordCount} words`,
          estimatedPages,
      ...
      if (file) {
        ...
        await pool.query("UPDATE books SET file_path=$1 WHERE id=$2", [
          fileName,
          bookId,
        ]);
      }
...
router.delete("/:id", authenticateToken, async (req, res) => {
  try {
    const book = (
      await pool.query(
        "SELECT id, file_path, uploaded_by FROM books WHERE id=$1",
        [req.params.id],
      )
    ).rows[0];
    if (!book) return res.status(404).json({ error: "Book not found" });
    ...
    await pool.query("DELETE FROM reading_sessions WHERE book_id=$1", [
      req.params.id,
    ]);
    await pool.query("DELETE FROM reading_progress WHERE book_id=$1", [
      req.params.id,
    ]);
    await pool.query("DELETE FROM chapters WHERE book_id=$1", [req.params.id]);
    await pool.query("DELETE FROM daily_streaks WHERE user_id=$1", [
      req.user.id,
    ]);
